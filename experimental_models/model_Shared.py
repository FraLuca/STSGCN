    ...
    def forward(self, x, A, T):
       ...
       # assert A.shape[0]==self.kernel_size, print(A.shape,self.kernel_size)
        if self.is_first:
            ...
            return x.contiguous(),self.A,self.T
        else:
            x = torch.einsum('nctv,vtq->ncqv', (x,T))
            x = torch.einsum('nctv,tvw->nctw', (x,A))

            return x.contiguous(),A,T
...
class ST_GCNN_layer(nn.Module):
    ...
    def __init__(self,
        ...
        self.gcn=ConvTemporalGraphical(time_dim,joints_dim,is_first_layer) # the convolution layer
...
class Model(nn.Module):
    ...
    def __init__(self,
        ...
        self.st_gcnns.append(ST_GCNN_layer(input_channels,64,[1,1],1,input_time_frame,
                                           joints_to_consider,st_gcnn_dropout,is_first_layer=True))
